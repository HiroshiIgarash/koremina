...
const FavoriteLiversForm = ({ user, setOpen: setOpenDialog }: FavoriteLiversFormProps) => {
  const [isPending, startTransition] = useTransition()
  const [livers, setLivers] = useState<Liver[]>([])

  const inputRef = useRef<HTMLInputElement>(null);
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<Liver[]>(()=>(user?.favoriteLivers || []));
  ...
  function handleSubmit(liversId:string[]) {
    startTransition(async () => {
      const newMostFavoriteLiver = await updateFavoriteLivers(liversId)
