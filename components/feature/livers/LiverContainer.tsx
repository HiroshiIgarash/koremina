...
const LiverContainer = async () => {
  ...
  // ライバー（海外）
  const liversOverseas = livers.filter(liver => liver.isOverseas && !liver.isRetire)
      ...
      <div className="px-4 w-full max-w-5xl mx-auto scroll-mt-5">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <Button asChild variant="secondary">
            <Link href="#jp" className="flex items-center gap-1"><ArrowDownCircle size={"1.2em"} />ライバー（JP）</Link>
          </Button>
          <Button asChild variant="secondary">
            <Link href="#overseas" className="flex items-center gap-1"><ArrowDownCircle size={"1.2em"} />ライバー（海外）</Link>
          </Button>
          <Button asChild variant="secondary">
            <Link href="#retired" className="flex items-center gap-1"><ArrowDownCircle size={"1.2em"} />卒業ライバー</Link>
          </Button>
        </div>
        <div className="flex flex-col gap-20">
          <LiverList livers={liversJp} heading="ライバー（JP）" user={user} id="jp" />
          <LiverList livers={liversOverseas} heading="ライバー（海外）" user={user} id="overseas" />
          <LiverList livers={liversRetired} heading="卒業ライバー" user={user} id="retired" />
        </div>
      </div>
    </>
  )
...
const LiverList = ({ livers, heading, user, id }: {
  livers: Liver[];
  heading: string;
  ...
  return (
    <div id={id}>
      <h2 className="font-bold text-2xl border-b-4 border-current pb-4">{heading}</h2>
          ...
          livers.map((liver) => (
            <div key={liver.id} className="relative flex flex-col items-center">
              {user?.mostFavoriteLiverId === liver.id && (
                ...
                <Heart fill="red" stroke="pink" className="absolute t-0 left-0 z-10 size-1/3 aspect-square" size={"100%"} />

              )}
              {user?.favoriteLivers.some(fav => fav.id === liver.id && fav.id !== user.mostFavoriteLiverId) && (

                <Star fill="yellow" stroke="orange" className="absolute t-0 left-0 z-10 size-1/3 aspect-square" size={"100%"} />
